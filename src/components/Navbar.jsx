    ...
    justify-between items center navbar">
        ...
        <ul className='list-none sm:flex hidden justify-end items-center flex-1'>
          {navLinks.map((nav, index) => (
            ...
            text-[16px] ${index === navLinks.lenght - 1 ? 'mr-0' : 'mr-10'} text-white`}>
              <a href={`#${nav.id}`}>
                {nav.title}
              </a>
            </li>
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center'>
          ...
          right-0 mx-4 my-2 min-w-[140px] rounded-xl sidebar `}>
              <ul className='list-none flex-col justify-end items-center flex-1 text-center'>
                {navLinks.map((nav, index) => (
                  <li key={nav.id} 
                  className={`font-poppins font-normal cursor-pointer
                  text-[16px] ${index === navLinks.lenght - 1 ? 'mr-0' : 'mb-4'} text-white`}>
                    <a href={`#${nav.id}`}>
                      {nav.title}
                    </a>
                  </li>
